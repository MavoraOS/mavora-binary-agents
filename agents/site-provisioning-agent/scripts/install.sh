...
write_config() {
    ...
    # agent_id = CN of the CA-signed leaf cert. The BE forces the CN to its own agent
    # UUID (overriding the CSR's CN), so the cert is the authoritative source (used for
    # job-envelope verification). AGENT_ID_GENERATED can't be used: generate_mtls_keypair
    # runs in a command-substitution subshell, so its global assignment is lost here.
    local agent_id
    agent_id="$(openssl x509 -in "$TLS_CERT" -noout -subject -nameopt RFC2253 2>/dev/null | sed -n 's/.*CN=\([^,]*\).*/\1/p')"
    [[ -n "$agent_id" ]] || fail "Could not extract agent_id (CN) from signed cert: $TLS_CERT"
